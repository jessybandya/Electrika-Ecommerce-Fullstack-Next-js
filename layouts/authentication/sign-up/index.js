...
function SignUp() {
  ...
  const completeRegistration = () => {
    ...
    if (!firstName) {
      ...
      toast.error('National ID No. is required!');
      setLoading(false);
      return;
    } else if (!password) {
  ...
  return (
    ...
    <meta name="description" content="Sign up page for Electrika Shops" />
    ...
    <Box sx={{ pb: 7 }} ref={ref} style={{marginBottom:-170}}>
    <CssBaseline />
...
<SoftBox mb={1} style={{display:'flex'}}>
<SoftInput type="text" value={idNo} onChange={e => setIdNo(e.target.value)} placeholder="National ID No."/>
</SoftBox>
  ...
  placeholder="Confirm Password" />
      ...
      <SoftBox mt={2} mb={1}>
        <SoftButton style={{backgroundColor:'#2152ff',color:'#fff'}} fullWidth onClick={completeRegistration}>
        {loading === true ?(
          <span><span style={{color:'#fff'}}>signing up...</span></span>
        ):(
          <span>Sign Up</span>
        )}  
        </SoftButton>
...
>
  Already have Eleckrika Computers Account?
</SoftTypography>
  </Link>
  </SoftTypography>
</SoftBox>
